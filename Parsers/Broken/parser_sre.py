...
# AUTO-NORMALIZE-SHIM v1
def _normalize_parser_output__v1(data):
    """
    Standardize parser returns to:
      { "po_number": str, "customer_name": str, "delivery_address": str }
    Accepts a variety of legacy shapes and pulls from best-known keys.
    """
    def _val(d, *keys):
        for k in keys:
    ...
    if isinstance(data, dict) and (
        ('po_number', 'customer_name', 'delivery_address') <= set(data.keys())
    ):
        # Ensure strings
        return {
            "po_number": _val(data, "po_number"),
            "customer_name": _val(data, "customer_name"),
            "delivery_address": _val(data, "delivery_address"),
        }

    # header/lines style
    if isinstance(data, dict) and ("header" in data or "lines" in data):
        header = data.get("header") or {}
        return {
            "po_number": _val(header, "po_number", "po", "purchase_order", "po-no", "order"),
            "customer_name": _val(header, "customer_name", "customer", "sold_to", "buyer"),
            "delivery_address": _val(header, "delivery_address", "ship_to", "anlieferadresse", "lieferadresse", "destinatario", "destination")
        }

    # list of dicts (lines) with a header-ish first element
    if isinstance(data, list) and data and isinstance(data[0], dict):
        h = data[0]
        return {
            "po_number": _val(h, "po_number", "po", "purchase_order"),
            "customer_name": _val(h, "customer_name", "customer", "sold_to", "buyer"),
            "delivery_address": _val(h, "delivery_address", "ship_to")
        }

    # plain string -> assume it's the delivery address only
    if isinstance(data, str):
        return {
            "po_number": "",
            "customer_name": "",
            "delivery_address": data.strip()
        }
...
# Wrap the first parse_* function found so it always returns the normalized shape.
try:
    _parse_names = [n for n, obj in globals().items() if n.startswith("parse_") and callable(obj)]
    if _parse_names:
        _orig_name = _parse_names[0]
        _orig_fn = globals()[_orig_name]
        def _wrapped_parse__v1(*args, **kwargs):
            out = _orig_fn(*args, **kwargs)
            return _normalize_parser_output__v1(out)
        globals()[_orig_name] = _wrapped_parse__v1
except Exception as _shim_exc:
    # Silent: we don't want parser import to fail because of shim
    pass
# END # AUTO-NORMALIZE-SHIM v1
