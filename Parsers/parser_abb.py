...
def detect_abb(text: str) -> bool:
    """
    Strict detection for ABB Inc. purchase orders.

    We ONLY activate this parser when the PO clearly identifies:
      - ABB Inc.
      - ABB Robotics
      - Auburn Hills, MI address
      - ABB Supply Unit

    This prevents misrouting caused by generic triggers like
    'purchase order' or 'vendor number'.
    """
    if not text:
        return False
...
def _extract_delivery_address(text: str) -> str:
    """
    Extract 'Ship To:' block. If missing, fallback to ABB Robotics Supply Unit.
    """
    m = re.search(
